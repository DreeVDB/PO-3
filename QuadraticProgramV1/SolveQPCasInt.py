...
def _build_solver(n, m, k, tolerance):
    ...
    options = {
        "ipopt.print_level": 0,
        "print_time": False,
        "ipopt.tol": float(tolerance),
        "ipopt.acceptable_tol": float(tolerance),
        "ipopt.constr_viol_tol": float(tolerance),
        "ipopt.compl_inf_tol": float(tolerance),
        "ipopt.dual_inf_tol": float(tolerance),
        "ipopt.max_iter": 2000,
    }
...
def SolveQPCasInt(Q, c, A, b, Aeq, beq, x0=None, return_stats=False, tolerance=1e-5):
    ...
    m = A.shape[0]
    k = Aeq.shape[0]
    solver, lbg, ubg = _build_solver(n, m, k, tolerance)
