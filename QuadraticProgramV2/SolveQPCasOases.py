...
def _build_solver(n, m, k, tolerance):
    cache_key = (n, m, k, float(tolerance))
    if cache_key in _SOLVER_CACHE:
        return _SOLVER_CACHE[cache_key]

    x = ca.SX.sym("x", n)
    parameter_size = n * n + n + m * n + m + k * n + k
    p = ca.SX.sym("p", parameter_size)
    ...
    constraints = ca.vertcat(*constraint_blocks) if constraint_blocks else ca.SX.zeros(0, 1)
    qp = {"x": x, "p": p, "f": objective, "g": constraints}

    options = {
        "printLevel": "none",
        "nWSR": 1000,
        "terminationTolerance": float(tolerance),
    }

    solver = ca.qpsol(f"solver_qpoases_{n}_{m}_{k}", "qpoases", qp, options)
    cached = (solver, lbg, ubg, [-ca.inf] * n, [ca.inf] * n)
...
def SolveQPCasOases(
    ...
    m = A.shape[0]
    k = Aeq.shape[0]
    solver, lbg, ubg, lbx, ubx = _build_solver(n, m, k, tolerance)
