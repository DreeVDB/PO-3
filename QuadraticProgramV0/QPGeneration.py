...
def Generate_QP_dataset(
    ...
    for _ in range(samples):
        ...
        if return_solver_stats:
            x, stats = solver_fn(Q, c, A, b, Aeq, beq, return_stats=True, tolerance=solver_tolerance)
            dataset.append([(Q, c, A, b, Aeq, beq), x, stats])
        else:
            x = solver_fn(Q, c, A, b, Aeq, beq, tolerance=solver_tolerance)
